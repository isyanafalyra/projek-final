<x-app-layout>
    ...
    <div class="tab-content" id="sidebarDashboardContent">
        ...
        <div class="tab-pane fade show active" id="overview-content">
            <div class="glass-card p-5 mb-4 text-center text-md-start">
                <div class="row align-items-center">
                    <div class="col-md-8">
                        <h4 class="text-white fw-bold mb-2">Sistem Intelijen Rantai Pasok Terpadu</h4>
                        <p class="text-secondary mb-4 mb-md-0">Selamat datang di dashboard GlobalChain. Pantau indikator makroekonomi, risiko cuaca pelabuhan, volatilitas valuta asing, dan sentimen berita logistik global di satu dashboard terintegrasi. Gunakan sidebar sebelah kiri untuk menelusuri detail analisis.</p>
                    </div>
                    <div class="col-md-4 text-center">
                        <i class="fa-solid fa-globe-asia text-info fa-5x animate-pulse"></i>
                    </div>
                </div>
            </div>
            
            <!-- Quick Summary Statistics Cards (Matches User Screenshot) -->
            <div class="row mb-4">
                ...
                <div class="col-lg-3 col-sm-6 mb-3 mb-lg-0">
                    <div class="status-card status-card-blue">
                        ...
                        <div class="status-card-content">
                            ...
                            <div class="status-card-subtext">Negara Aktif</div>
                        </div>
                    </div>
                </div>
                <!-- Card 2: Country Risk Status -->
                <div class="col-lg-3 col-sm-6 mb-3 mb-lg-0">
                    <div class="status-card status-card-red">
                        ...
                        <div class="status-card-content">
                            <div class="status-card-title">Country Risk Status</div>
                            <div class="status-card-value text-danger" id="statHighRiskCount">52.2% (Medium)</div>
                            <div class="status-card-subtext">Tingkat Kerawanan</div>
                        </div>
                    </div>
                </div>
                <!-- Card 3: Ports in Country -->
                <div class="col-lg-3 col-sm-6 mb-3 mb-lg-0">
                    <div class="status-card status-card-yellow">
                        ...
                        <div class="status-card-content">
                            <div class="status-card-title">Ports in Country</div>
                            <div class="status-card-value" id="statPortsCount">0</div>
                            <div class="status-card-subtext" id="statPortsSubtext">Pelabuhan Aktif</div>
                        </div>
                    </div>
                </div>
                <!-- Card 4: Local Wind & Temp -->
                <div class="col-lg-3 col-sm-6">
                    <div class="status-card status-card-green">
                        ...
                        <div class="status-card-content">
                            <div class="status-card-title">Local Wind & Temp</div>
                            <div class="status-card-value" id="statWeatherSummary">27 km/h</div>
                            <div class="status-card-subtext" id="statWeatherTemp">(15°C)</div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="glass-card p-4">
                <h5 class="text-white fw-bold mb-3"><i class="fa-solid fa-circle-info text-info me-2"></i>Panduan Monitoring Global</h5>
                <p class="text-secondary small mb-0" style="line-height: 1.6;">
                    Gunakan panel navigasi di sebelah kiri untuk mengakses berbagai instrumen intelijen secara mendalam. Di bagian <strong>Analytics</strong>, Anda dapat meninjau profil lengkap negara, kalkulasi bobot risiko multisektoral, peta monitor cuaca maritim real-time, dan dampak fluktuasi nilai tukar. Di bagian <strong>Intelligence</strong>, platform ini menyediakan sentimen analisis berita terkini, peta pelabuhan kargo logistik utama, serta visualisasi perbandingan makroekonomi antar negara secara komparatif.
                </p>
            </div>
        </div>
        ...
        <script>
            ...
            window.addEventListener('DOMContentLoaded', () => {
                ...
                document.getElementById('mapResetBtn').addEventListener('click', () => {
                    ...
                    document.getElementById('countryTriggerLabel').textContent = 'Pilih Negara...';
                    document.getElementById('triggerFlagContainer').innerHTML = '<i class="fa-solid fa-globe text-info"></i>';
                    // Remove selected highlight
                    document.querySelectorAll('#countryDropdownList .country-dropdown-item').forEach(el => el.classList.remove('selected'));
                });
                ...
                document.getElementById('weatherMapResetBtn').addEventListener('click', () => {
                    ...
                    selectedWeatherCountryCode = '';
                    document.getElementById('weatherCountryTriggerLabel').textContent = 'Pilih Negara...';
                    document.getElementById('weatherTriggerFlagContainer').innerHTML = '<i class="fa-solid fa-globe text-info"></i>';
                    document.querySelectorAll('#weatherCountryDropdownList .country-dropdown-item').forEach(el => el.classList.remove('selected'));
                });
            ...
            function renderCountryDropdownItems(list) {
                ...
                container.querySelectorAll('.country-dropdown-item').forEach(item => {
                    item.addEventListener('click', () => {
                        ...
                        // Close dropdown and trigger map change
                        closeCountryDropdown();
                        handleMapCountryChange(isoCode);
                    });
                });
            }
            ...
            function renderWeatherCountryDropdownItems(list) {
                ...
                container.querySelectorAll('.country-dropdown-item').forEach(item => {
                    item.addEventListener('click', () => {
                        ...
                        closeWeatherCountryDropdown();
                        handleWeatherMapCountryChange(isoCode);
                    });
                });
            }
                    ...
                    .then(response => {
                        if (response.status === 'success') {
                            ...
                            if (countriesList.length > 0) {
                                ...
                                const defaultCountryCode = idIndex !== -1 ? countriesList[idIndex].iso_code : countriesList[0].iso_code;
                ...
                fetch('/api/ports')
                    ...
                    .then(response => {
                        if (response.status === 'success') {
                            ...
                            // Port count per country is handled by syncPortsForCountry() instead of global total
                            
                            // Fetch default weather for Card 4 using the first port
                            if (response.data.length > 0) {
                                const firstPort = response.data[0];
                                fetch(`https://api.open-meteo.com/v1/forecast?latitude=${firstPort.lat}&longitude=${firstPort.lng}&current=temperature_2m,wind_speed_10m`)
                                    .then(res => res.json())
                                    .then(weatherData => {
                                        if (weatherData && weatherData.current) {
                                            const cur = weatherData.current;
                                            document.getElementById('statWeatherSummary').textContent = `${Math.round(cur.wind_speed_10m)} km/h`;
                                            document.getElementById('statWeatherTemp').textContent = `(${Math.round(cur.temperature_2m)}°C)`;
                                        }
                                    })
                                    .catch(err => console.error("Error fetching card weather:", err));
                            }
                        }
                    })
            ...
            function handleCountryChange(countryCode) {
                ...
                // 1. Update basic profile info instantly from memory (instant feedback, zero loading lag!)
                updateLocalProfile(countryCode);

                // 2. Sync ports stat card & sidebar info with new country
                syncPortsForCountry(countryCode);
                
                // 3. Fetch data lazily depending on the currently active view tab
                ...
                fetch(`/api/risk?country_code=${countryCode}`)
                    ...
                    .then(response => {
                        if (response.status === 'success') {
                            ...
                            // Re-calculate statistics for High-Risk countries count
                            updateHighRiskStats();
                        }
            ...
            function viewCountryFromWatchlist(isoCode) {
                currentCountryCode = isoCode;
                document.getElementById('countrySelect').value = isoCode;
                switchTab('#country-profile-content');
                updateLocalProfile(isoCode);
            }
            ...
            function updateHighRiskStats() {
                ...
                Object.keys(analyticsCache).forEach(code => {
                    const risk = analyticsCache[code].risk;
                    if (risk) {
                        calculatedCount++;
                        if (risk.scores.total > 60) highRiskCount++;
                    }
                });
                ...
                if (highRiskCountEl) {
                    if (calculatedCount > 0) {
                        ...
                        Object.keys(analyticsCache).forEach(code => {
                            const risk = analyticsCache[code].risk;
                            if (risk) {
                                sumTotal += risk.scores.total;
                            }
                        });
                        const avgTotal = sumTotal / calculatedCount;
                        let avgLevel = 'Low';
                        if (avgTotal > 60) avgLevel = 'High';
                        else if (avgTotal >= 30) avgLevel = 'Medium';
                        highRiskCountEl.innerHTML = `${avgTotal.toFixed(1)}% <span class="small text-secondary">(${avgLevel})</span>`;
                    } else {
                        highRiskCountEl.textContent = '52.2% (Medium)';
                    }
                }
            }
